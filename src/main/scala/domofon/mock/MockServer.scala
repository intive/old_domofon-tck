...
import akka.http.scaladsl.server.{ValidationRejection, RejectionHandler, Directives, Route}
...
trait MockServer extends Directives with SprayJsonSupport with Marshalling {
  ...
  private[this] val rejectionHandler: RejectionHandler = RejectionHandler.newBuilder().handle {
    case vr: ValidationRejection =>
      complete(
        HttpResponse(StatusCodes.UnprocessableEntity, entity = vr.message + vr.cause.map(c => ": " + c.getMessage).getOrElse(""))
      )
  }.result()
            ...
            json =>
              Try(json.convertTo[ContactRequest]) match {
                case Success(contact) =>
                  ...
                  contacts.update(id, ContactResponse.from(contact))
                  complete(id)
                case Failure(ex) =>
                  reject(ValidationRejection("Contact requests has wrong structure", Some(ex)))
              }
          }
